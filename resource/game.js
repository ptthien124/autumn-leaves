Physics(function (world) {
    ...
    var branch = function (parent, i, nMax, branchVec) {
      ...
      if (i < nMax) {
        ...
        var b = branch(
          ...
          i + 1,
          nMax,
          branchVec.rotate(trans.setRotation(2 * theta)).clone()
        );
        ...
        var leaf = Physics.body("rectangle", {
          width: 10,
          height: 6,
          mass: 1,
          angle: Math.random(),
        ...
        constraints.distanceConstraint(particle, leaf, 0.1);
        leaf.leaf = true;
        leaf.attached = true;
        nodes.push(leaf);
      }
  ...
  [
    [
      { x: renderer.width / 2, y: renderer.height - 10 },
      6,
      110,
      0.9,
      Math.PI / 2 / 3,
    ...
    [
      { x: renderer.width / 2 + 300, y: renderer.height - 10 },
      5,
      50,
      0.9,
      Math.PI / 2 / 3,
    ...
    world.on(
      ...
      function () {
        ...
        for (var i = 0, l = constrs.length; i < l; ++i) {
          ...
          if (
            leaf &&
            ((leaf.state.vel.norm() > threshold && Math.random() > 0.99) ||
              Math.random() > 0.9999)
          ) {
        ...
        for (var i = 0, l = bodies.length; i < l; i++) {
          ...
          if (body.leaf) {
            ...
            if (cutoff && body.state.pos.get(1) < cutoff) {
              body.applyForce(
                dir.clone({
                  x: Math.cos(this.theta) * r,
                  y: Math.sin(this.theta) * r - (0.0004 - 0.00004) * body.mass,
                }),
                tmp
              );
            }
          ...
          body.state.angular.vel = Math.min(
            Math.max(body.state.angular.vel, -0.01),
            0.01
          );
        }
